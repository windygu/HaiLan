        ...
        private void LoadDocInfo()
        {
            ...
            if (true)
            {
                thread = new Thread(new ThreadStart(() =>
                {
                    ...
                    {
                        if (parentForm != null)
                            parentForm.loadBasicInfo(new DocInfo() { DOCNO = "18291",ZYPXFLG="X" }, new List<DocDetailInfo>(), new List<MaterialInfo>(), new List<HLATagInfo>(), new List<EpcDetail>(), new List<HuInfo>(), type, new List<MixRatioInfo>());
                        this.Close();
                        return;
                    }
                    else if (docNo == "y")
                    {
                        if (parentForm != null)
                            parentForm.loadBasicInfo(new DocInfo() { DOCNO = "18291" }, new List<DocDetailInfo>(), new List<MaterialInfo>(), new List<HLATagInfo>(), new List<EpcDetail>(), new List<HuInfo>(), type, new List<MixRatioInfo>());
                        this.Close();
                        return;
                    }
                    #endregion
                    ...
                    //LogService.Log(SysConfig.DeviceNO,loginfo);
                    OnThreadReturn();
                    this.Invoke(new Action(() => {
                        if (parentForm != null)
